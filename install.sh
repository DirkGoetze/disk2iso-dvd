#!/bin/bash
# ===========================================================================
# disk2iso-dvd Module Installation Script
# ===========================================================================
# Installation script for Video-DVD Ripping Module
# ===========================================================================
...
MISSING_DEPS=""
for cmd in dvdbackup gddrescue; do
...
if [ -n "$MISSING_DEPS" ]; then
    ...
    apt-get update
    apt-get install -y dvdbackup gddrescue libdvd-pkg
    # Configure libdvdcss2
...
# Copy library
cp -v lib/libdvd.sh "$INSTALL_DIR/lib/"
chmod 755 "$INSTALL_DIR/lib/libdvd.sh"

# Copy configuration
cp -v conf/libdvd.ini "$INSTALL_DIR/conf/"
chmod 644 "$INSTALL_DIR/conf/libdvd.ini"

# Copy language files
cp -v lang/libdvd.* "$INSTALL_DIR/lang/"
chmod 644 "$INSTALL_DIR/lang/libdvd".*

# Copy documentation
echo -e "${YELLOW}Installing documentation...${NC}"
cp -rv doc/04_Module "$INSTALL_DIR/doc/"
chmod 644 "$INSTALL_DIR/doc/04_Module/04-2_DVD-Video.md"
...
echo "3. View documentation: cat $INSTALL_DIR/doc/04_Module/04-2_DVD-Video.md"
echo ""
